import { restaurants } from '../../restaurants.json';
import filterRestaurants from './filterRestaurants';

describe('filterRestaurants', () => {
  it('식당이름 검색어로 레스토랑을 필터링 한다.', () => {
    const searchKeyword = '메가';
    const result = [
      {
        id: '1',
        category: '중식',
        name: '메가반점',
        menu: [
          { id: '1', name: '짜장면', price: 8000 },
          { id: '2', name: '짬뽕', price: 8000 },
          { id: '3', name: '차돌짬뽕', price: 9000 },
          { id: '4', name: '탕수육', price: 14000 },
        ],
      },
      {
        id: '6',
        category: '한식',
        name: '메가김치찌개',
        menu: [
          { id: '22', name: '김치찌개1인', price: 8000 },
          { id: '23', name: '된장찌개', price: 8000 },
          { id: '24', name: '계란말이', price: 6000 },
        ],
      },
    ];

    expect(filterRestaurants(restaurants, { searchText: searchKeyword })).toEqual(result);
  });

  it('카테고리로 레스토랑을 필터링 한다.', () => {
    const category = '한식';
    const result = [
      {
        id: '2',
        category: '한식',
        name: '메리김밥',
        menu: [
          { id: '5', name: '김밥', price: 3500 },
          { id: '6', name: '참치김밥', price: 4500 },
          { id: '7', name: '제육김밥', price: 5000 },
          { id: '8', name: '훈제오리김밥', price: 5500 },
          { id: '9', name: '컵라면', price: 2000 },
        ],
      },
      {
        id: '3',
        category: '한식',
        name: '데브부엌',
        menu: [
          { id: '10', name: '제육덮밥', price: 10000 },
          { id: '11', name: '닭떡국', price: 9000 },
          { id: '12', name: '닭개장', price: 11000 },
          { id: '13', name: '돈까스', price: 1000 },
        ],
      },
      {
        id: '6',
        category: '한식',
        name: '메가김치찌개',
        menu: [
          { id: '22', name: '김치찌개1인', price: 8000 },
          { id: '23', name: '된장찌개', price: 8000 },
          { id: '24', name: '계란말이', price: 6000 },
        ],
      },
    ];

    expect(filterRestaurants(restaurants, { selectedCategory: category })).toEqual(result);
  });
});
